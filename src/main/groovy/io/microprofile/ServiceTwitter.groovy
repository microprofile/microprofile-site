...
class ServiceTwitter {
    ...
    @Inject
    @Config(value = 'microprofile_twitter_oauth_consumer_key')
    private String oAuthConsumerKey

    @Inject
    @Config(value = 'microprofile_twitter_oauth_consumer_secret')
    private String oAuthConsumerSecret

    @Inject
    @Config(value = 'microprofile_twitter_oauth_access_token')
    private String oAuthAccessToken

    @Inject
    @Config(value = 'microprofile_twitter_oauth_access_token_secret')
    ...
    Collection<DtoTweet> getTweets() {
        return this.twitter.search(new Query('#javaee')).tweets.collect { Status tweet ->
            return new DtoTweet(
                    id: tweet.id,
