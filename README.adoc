...
==== Amazon Architecture
```
[Elastic Load Balancer] |-> [Active Instances Subnet0] -> [Docker Instance]
...
This directory is filtered and copied to `/.ebextensions` in the root of the assembled ZIP file. It is not part of the JAR. These files configure the Elastic Beanstalk cluster.
